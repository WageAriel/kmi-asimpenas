...
const singleDeleteError = ref(null);
const singleDeleteSuccess = ref(null);
...
const deleteSingleItem = async () => {
    if (!itemToDelete.value || isSingleDeleting.value) return;
    ...
    try {
        await axios.delete(`/data-seleksi-mitra/${itemToDelete.value.id_seleksi_mitra}`);
        
        singleDeleteSuccess.value = 'Data seleksi mitra berhasil dihapus';
        ...
        if (error.response) {
            const data = error.response.data;
            singleDeleteError.value = data.message || 'Terjadi kesalahan saat menghapus data';
        } else {
            singleDeleteError.value = `Terjadi kesalahan: ${error.message}`;
        }
    } finally {
...
<template>
    ...
    <AdminLayout>
        ...
        <div v-if="showSingleDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" @click="closeSingleDeleteModal">
            <div class="bg-white rounded-xl shadow-lg max-w-md w-full p-6 relative" @click.stop>
                <button @click="closeSingleDeleteModal" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
                ...
                <div class="text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2">Hapus Data Seleksi Mitra</h3>
                    <p class="text-sm text-gray-500 mb-4">
                        Apakah Anda yakin ingin menghapus data seleksi mitra 
                        <span class="font-semibold">{{ itemToDelete?.mitra?.nama_perusahaan }}</span>?
                        <br>
                        <span class="text-red-600">Tindakan ini tidak dapat dibatalkan.</span>
                    </p>

                    <!-- Success Message -->
                    <div v-if="singleDeleteSuccess" class="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                        <div class="flex items-center justify-center">
                            <svg class="h-5 w-5 text-green-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            <p class="text-sm font-medium text-green-800">{{ singleDeleteSuccess }}</p>
                        </div>
                    </div>

                    <!-- Error Message -->
                    <div v-if="singleDeleteError" class="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                        <div class="flex items-start justify-center">
                            <svg class="h-5 w-5 text-red-400 mr-2 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
                            </svg>
                            <p class="text-sm font-medium text-red-800">{{ singleDeleteError }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-center gap-3 mt-6">
                        ...
                        <button
                            ...
                            :disabled="isSingleDeleting"
                        >
                            <span v-if="isSingleDeleting" class="flex items-center">
                                <svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
