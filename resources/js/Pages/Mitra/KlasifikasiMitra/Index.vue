...
onMounted(async () => {
    ...
    try {
        ...
        classifications.value = items.map(item => {
            ...
            return {
                ...item,
                // Normalisasi field untuk tabel
                id: item.id_klasifikasi_mitra ?? item.id,
                nama_perusahaan: item.mitra?.nama_perusahaan ?? '-',
                year: createdYear ?? currentYear,
                // Status klasifikasi: default 'Belum Diverifikasi', menjadi 'Sudah Diverifikasi' jika hasil_klasifikasi ada
                status: item.hasil_klasifikasi ? 'verified' : 'unverified',
                // Hasil klasifikasi A/B/C dari backend
    ...
    // Ambil data seleksi milik user (menentukan apakah boleh klasifikasi)
    try {
        const resSeleksi = await axios.get('/data-seleksi-mitra/my');
        selections.value = Array.isArray(resSeleksi.data) ? resSeleksi.data : [];
    } catch (e) {
        // Silent fail
    }
});
...
const getStatusClass = (status) => {
    switch (status) {
        case 'verified': return 'bg-green-100 text-green-800';      // Sudah Diverifikasi
        case 'unverified': return 'bg-yellow-100 text-yellow-800';  // Belum Diverifikasi
...
const getStatusText = (status) => {
    switch (status) {
        ...
        case 'unverified': return 'Belum Diverifikasi';
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                            ...
                            <tr v-for="classification in classifications" :key="classification.id"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span v-if="classification.result" :class="[
                                        ...
                                        classification.result === 'A' ? 'bg-green-100 text-green-800' :
                                        classification.result === 'B' ? 'bg-yellow-100 text-yellow-800' :
                                ...
                                <!-- <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-sm">
                                        <span class="font-bold text-blue-600">{{ classification.total_score || 0
                                            }}</span>
                                ...
                                <!-- <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="w-full bg-gray-200 rounded-full h-2">
                                        <div :class="[
                                            ...
                                            :style="`width: ${Math.min(((classification.total_score || 0) / 75) * 100, 100)}%`">
                                        </div>
                                    </div>
                                </td> -->
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <button @click="viewDetails(classification)"
                                            class="text-blue-600 hover:text-blue-900 transition-colors">
                                            Lihat
                                        </button>
                                        <button v-if="classification.status === 'draft'"
                                            @click="editClassification(classification)"
                                            class="text-green-600 hover:text-green-900 transition-colors">
                                            Lanjutkan
                                        </button>
                                        <button v-if="classification.status === 'completed'"
                ...
                <div v-else class="text-center py-12">
                    <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                        <path d="M9 12h6m6 0h6m-6 6v6m-6-6v6m6-6v6" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round" />
                    </svg>
                    <h3 class="mt-2 text-sm font-medium text-gray-900">Belum ada klasifikasi</h3>
                    <p class="mt-1 text-sm text-gray-500">
                        <span v-if="!hasApprovedSubmission">Selesaikan pengajuan seleksi terlebih dahulu untuk dapat
                ...
                <div v-if="selectedClassification" class="overflow-y-auto flex-1 pr-2">
                    ...
                    <div class="mb-4 grid grid-cols-2 gap-x-6 gap-y-2">
                        ...
                        <div>
                            ...
                            <span class="font-semibold text-gray-900">{{ selectedClassification.year || currentYear
                                }}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <span class="font-semibold text-gray-900">{{ formatDate(selectedClassification.created_at)
                                }}</span>
                        </div>
                    </div>
                    <hr class="my-4">
                        ...
                        <div>
                            ...
                            <div class="mb-2">
                                ...
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pembersih_gabah',
                                    selectedClassification.mesin_pembersih_gabah) }} (ton/hari)</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Lantai Jemur</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('lantai_jemur', selectedClassification.lantai_jemur) }}
                                    (ton/hari)</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pengering</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pengering', selectedClassification.mesin_pengering)
                                    }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Tangki Penyimpanan</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('alat_pengering_lainnya',
                                    selectedClassification.alat_pengering_lainnya) }}</span>
                            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('gudang_konvensional',
                                    selectedClassification.gudang_konvensional) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Silo GKG/Hopper</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('silo_gkg_hopper', selectedClassification.silo_gkg_hopper)
                                    }}</span>
                            </div>
                            <div class="mb-3 font-semibold text-green-700 pb-1">Sarana Angkut</div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Truk</span>
                                <span class="font-medium text-gray-900 block mt-1">{{ interpretClassification('truk',
                                    selectedClassification.truk ) }}</span>
                            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mini_lab', selectedClassification.mini_lab) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Moisture Tester</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('moisture_tester', selectedClassification.moisture_tester)
                                    }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Pembanding Derajat Sosoh</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('pembanding_derajat_sosoh',
                                    selectedClassification.pembanding_derajat_sosoh) }}</span>
                            </div>
                            ...
                            <div class="mb-2">
                                ...
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('bagian_quality_control',
                                    selectedClassification.bagian_quality_control) }}</span>
                            </div>
                        </div>
                        <!-- Kolom Kiri -->
                        <div>
                            ...
                            <div class="mb-2">
                                ...
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pembersih_awal',
                                    selectedClassification.mesin_pembersih_awal) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemecah Kulit</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemecah_kulit',
                                    selectedClassification.mesin_pemecah_kulit) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pembersih Sekam</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pembersih_sekam',
                                    selectedClassification.mesin_pembersih_sekam) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemisah Gabah Pecah Kulit</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemisah_gabah_pecah_kulit',
                                    selectedClassification.mesin_pemisah_gabah_pecah_kulit) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemisah Batu</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemisah_batu',
                                    selectedClassification.mesin_pemisah_batu) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Penyosoh</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_penyosoh', selectedClassification.mesin_penyosoh)
                                    }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pengkabut</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pengkabut', selectedClassification.mesin_pengkabut)
                                    }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemesah Menir</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemesah_menir',
                                    selectedClassification.mesin_pemesah_menir) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemisah Katul</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemisah_katul',
                                    selectedClassification.mesin_pemisah_katul) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemisah Berdasarkan Ukuran</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemisah_berdasarkan_ukuran',
                                    selectedClassification.mesin_pemisah_berdasarkan_ukuran) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pemisah Berdasarkan Warna</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pemisah_berdasarkan_warna',
                                    selectedClassification.mesin_pemisah_berdasarkan_warna) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Tangki Penyimpanan</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('tangki_penyimpanan',
                                    selectedClassification.tangki_penyimpanan) }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Pengemas</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_pengemas', selectedClassification.mesin_pengemas)
                                    }}</span>
                            </div>
                            <div class="mb-2">
                                <span class="text-sm text-gray-500">Mesin Jahit</span>
                                <span class="font-medium text-gray-900 block mt-1">{{
                                    interpretClassification('mesin_jahit', selectedClassification.mesin_jahit) }}</span>
                            </div>
                    ...
                    <div class="flex space-x-2 justify-end mt-6">
                        <button @click="closeModal"
                            class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Tutup</button>
                        <button @click="editClassification(selectedClassification)"
